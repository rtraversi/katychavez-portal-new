# ============================================================================
#  scroggins-sync-pickers-book.ps1  -  Port the searchable-pickers + /book
#  redesign batch into Scroggins & Savage (SSL). Written 2026-07-14.
#
#  RUN FROM ANY POWERSHELL WINDOW (self-contained, cd's to SSL):
#      C:\Sites\iurisiq-portal-template\scroggins-sync-pickers-book.ps1
#
#  WHAT IT PORTS (merged to template master, 7f02714):
#    - New Invoice: matter <select> replaced with a type-to-filter searchable
#      combobox (.dk-combo) - filter by client name or case number, keyboard
#      nav, match highlighting.
#    - Settings > Billing Rates: client <select> replaced with the same
#      searchable combobox (filter by client name).
#    - Public /book page rebuilt on the Docket kit: serif masthead, firm logo
#      (with same-origin fallback; hides if none), paper case-file cards,
#      brand-color-driven accent hue, attorney photos from account avatars.
#
#  *** DATABASE: NOTHING TO RUN (no migrations in this batch). ***
...
#  NOTE: css/portal.css is copied WHOLESALE (template is upstream; SSL carries
#  no client-specific edits in it - client branding lives in config/assets).
#
...
$tmpTar = Join-Path $env:TEMP ("ssl-pickers-book-sync-{0}.tar" -f $PID)
...
Write-Host "DATABASE - nothing to run." -ForegroundColor DarkGray
...
Write-Host "  1. Billing -> New Invoice: matter picker is now a search box - type a client" -ForegroundColor Gray
Write-Host "     name or case number, arrow keys + Enter work, match text highlights." -ForegroundColor Gray
Write-Host "  2. Settings -> Billing Rates: client picker is the same search box; picking a" -ForegroundColor Gray
Write-Host "     client still loads their rates + admin fee exactly as before." -ForegroundColor Gray
Write-Host "  3. Public /book page: new Docket look. Check LIGHT and DARK." -ForegroundColor Gray
Write-Host "  4. /book logo: if the SSL logo is missing up top, set the firm's logo URL in" -ForegroundColor Gray
Write-Host "     Settings -> Scheduling (branding). Fallback chain: firm logo_url ->" -ForegroundColor Gray
Write-Host "     assets/iurisiq-logo.png -> hidden (never a broken image)." -ForegroundColor Gray
Write-Host "  5. Book a throwaway consult slot end-to-end to confirm the flow still submits." -ForegroundColor Gray
